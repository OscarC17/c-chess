...
int main(int argc, char * argv[]) {
	...
	for (int turncount = 0; ; turncount++) {
		...
		std::string specialmoves = "";

		std::cout << " % " << ((int)srcIndex / 8) << " % \n";

		if ((int)srcIndex % 8 == 1 || (int)srcIndex % 8 == 6) {
			specialmoves.append("f");
		}

		std::vector<int> moves = genTurn(chessboard, specialmoves, srcIndex);
